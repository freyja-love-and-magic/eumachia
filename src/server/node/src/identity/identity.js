...
// so it goes through the same filesystem-backed persistence client db.js uses
// elsewhere in the ecosystem.
//
// This used to switch on PERSISTENCE_BACKEND=netlify-blobs, because a Netlify
// Function's filesystem is read-only outside /tmp and every mkdir here ENOENTed.
// That bundle is retired and the Blobs client was dropped on extraction; the
// import stays lazy only because it's inside an async IIFE already.
const IDENTITY_KEY = 'identity:eumachia';

const client = (async () => {
  const { createClient } = await import('../persistence/client.js');
