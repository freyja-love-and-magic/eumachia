...
// The pay page's own JavaScript calls back to this service (/intent,
// /complete), and those routes are mounted here at /pay/:uuid/... while the
// browser sees them under whatever prefix the proxy mounts eumachia at.
// Behind allyabase's nginx (`location /eumachia/ { proxy_pass
// http://localhost:3013/; }`) the trailing slash strips /eumachia before it
// reaches Express, so the page has to put it back when it builds those URLs.
//
// Defaults to /eumachia, which is what that nginx route gives you and what
// this was previously hardcoded to. Set PUBLIC_PREFIX='' to run eumachia
// directly with no proxy in front, or to something else to mount it
// elsewhere. No trailing slash — the paths below supply it.
const PUBLIC_PREFIX = (process.env.PUBLIC_PREFIX ?? '/eumachia').replace(/\/$/, '');
    ...
    <script>
      ...
      async function startPayment() {
        await withPayButton(async () => {
          const resp = await fetch('${PUBLIC_PREFIX}/pay/${encodeURIComponent(uuid)}/intent', {
            method: 'POST',
      ...
      async function finalizeComplete() {
        const resp = await fetch('${PUBLIC_PREFIX}/pay/${encodeURIComponent(uuid)}/complete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(readCreds)
        });
...
// Same self-healing retry every other allyabase service uses for its own
// bootstrap call (see bdo.js's `repeat`/`bootstrap`). eumachia's identity
// bootstrap has to reach bdo and addie, and nothing orders service startup:
// under pm2 all of them come up at once, so the first attempt can lose the
// race with ECONNREFUSED. Without a retry, bdoUuid/addieUuid stay null for
// the life of the process, silently breaking every /pay/:uuid route after.
//
// (This originally described losing the race against the netlify-gateway
// bundle's startAll(). That bundle is gone; the race isn't.)
...
// on the droplet via pm2 — so the same `app` can still be imported by a test
// harness or an embedding host without starting a listener. Matches
// bdo.js's/addie.js's pattern.
//
